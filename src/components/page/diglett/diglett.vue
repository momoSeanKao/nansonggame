<template>
	<div class="game_box diglett-box">
		<!--游戏背景，分上下-->
		<div class="game-bg">
			<div class="game-top-bg">
				<img src="https://app.bowu66.com/museum-cms/upload/nansongGame/diglett/top-bg.png" alt="">
			</div>
			<div class="game-top-bottom">
				<img src="https://app.bowu66.com/museum-cms/upload/nansongGame/diglett/bottom-bg.png" alt="">
			</div>
		</div>
		<!--游戏容器-->
		<div class="game-tips-box">
			<!--游戏倒计时-->
			<div :class="['clock-box','clock-diglett',(totalTime<5&&(totalTime<5&&totalMs>0))?'clock-shine':'']">
				{{totalTime>9?totalTime:("0"+totalTime)}}:{{totalMs>9?totalMs:("0"+totalMs)}}
			</div>
			<div class="progress yellow">
				<div class="progress-bar" :style="{ width: accuracy+'%', background:'#fbad00'}">
					<!--<div class="progress-value">游戏进度{{accuracy}}%</div>-->
				</div>
			</div>

		</div>
		<div class="game_container">
			<!--步骤-->
			<div class="game_tips_box">
				<div class="game-step">
					<img src="https://app.bowu66.com/museum-cms/upload/nansongGame/img/step1.png" alt="">
				</div>
				<div class="box-shadow"></div>
			</div>
			<!--游戏操作区-->
			<div class="game_operation_box">
				<ul>
					<li v-for="(item,i) in diglettPlaceArr" :class="['itemList'+i]">
						<span v-bind:class="[item.active? 'active' : '']">{{item.name}}</span>

						<img :src="item.url" v-bind:class="[item.active? 'active' : '',item.truePic?'truePic':'']" :data-name="item.name" :data-index="i" @click="placeClick"></img>
					</li>
				</ul>
			</div>
		</div>

		<countDownT ref="countDownChild"></countDownT>
		<!--动画-->
		<div class="animation_box" v-show="animationShow">
			<div class="animation-bg2">
				<img src="https://app.bowu66.com/museum-cms/upload/nansongGame/diglett/game-2-bg2.png" alt="">
			</div>
			<div class="animation-bg3">
				<img src="https://app.bowu66.com/museum-cms/upload/nansongGame/diglett/game-2-bg3.png" alt="">
			</div>
			<div class="animation-bg4"></div>
			<div class="animation-bg5">
				<img src="https://app.bowu66.com/museum-cms/upload/nansongGame/diglett/game-2-bg5.png" alt="">
			</div>
			<div class="animation-bg6" @click="hideanimationShow"></div>
			<div class="animation-bg7">
				<img src="https://app.bowu66.com/museum-cms/upload/nansongGame/returnOwner/shine-bg1.png" alt="">
			</div>
		</div>
		<!--通过-->
		<!--<pass :passShow="passShow" :gotoShow="gotoShow" @gotoPage="gotoPage" :passWord="false"></pass>-->
		<!--失败-->
		<refuse :refuseShow="refuseShow" @restart="restart" @defaultGame="defaultGame" :showDefault="true"></refuse>
		<!--规则-->
		<stepRule :stepRule="stepRule" :stepTips="stepTips" :stepLogo="stepLogo" :stepTitle="stepTitle" ref="gameRule" @gameStart="gameStart"></stepRule>
		<audio id="diglett_click_sound_box">
			<source src="https://app.bowu66.com/museum-cms/upload/nansongGame/secondSound/second-click.mp3" type="audio/mp3">
		</audio>
		<audio id="time_sound">
			<source src="https://app.bowu66.com/museum-cms/upload/nansongGame/secondSound/five-times.mp3" type="audio/mp3">
		</audio>
		<!--采土清单-->
		<div class="mining-box" v-show="passShow">
			<!--<div class="mining-title">本次采料清单</div>-->
			<div class="mining-content">
				<!--<div class='mining-item'>-->
				<!--<div class='mining-name'>" + n.name + "</div>-->
				<!--<div class='mining-count'>X " + n.count + "</div>-->
				<!--</div>-->
			</div>
			<div class="next-step" @click="gotoPage">
				<img src="https://app.bowu66.com/museum-cms/upload/nansongGame/img/next-step.png" alt="">
			</div>
		</div>
	</div>
</template>

<script>
	var t1;
	var clickCount = 1;
	import countDownT from "../../common/countDown/countDown";
	import stepRule from "../../common/stepRule/stepRule";
	import pass from "../../common/pass/pass";
	import refuse from "../../common/refuse/refuse";

	export default {
		name: "diglett",
		data() {
			return {
				// 开场动画参数
				animationShow: true, //开场动画
				// 规则
				stepRule: "<div>制作陶瓷首先需要采集原料，并用粉碎、研磨、淘洗、炼制等手段对其加工。传统瓷器最主要的原料有<span class='organ-word'>瓷石、黏土、长石、石英、草木灰、石灰石以及白云石</span>等。" +
					"</div>",
				stepTips: "赶快点击屏幕上跳动的制瓷原料进行采集吧！",
				stepTitle: "【采土取样】",
				stepLogo: "<div class='icon-box'>" +
					"<img src='" + ('https://app.bowu66.com/museum-cms/upload/nansongGame/diglett/stone4.png') + "' alt=''>" +
					"</div>" +
					"<div class='hand-box'>" +
					"<img src='data:image/png;base64,' alt=''>" +
					"</div>",
				// 游戏参数
				canPlay: false, //是否可以开始游戏
				stayTime: 1000, //地鼠停留时间s
				correctCount: 0, //用户正确数量
				accuracy: 0, //正确率
				clickTotalCount: 0, //用户总点击数
				totalTime: 59, //游戏总时长
				totalMs: 99, //总毫秒数
				needClickCorrectCount: 20, //需要选择正确的材料的个数
				diglettPlaceArr: [], //随机处
				materialsList: [ //所有材料列表
					{
						url: ("https://app.bowu66.com/museum-cms/upload/nansongGame/diglett/stone1.png"),
						name: "草木灰",
						count: 0
					},
					{
						url: ("https://app.bowu66.com/museum-cms/upload/nansongGame/diglett/stone2.png"),
						name: "黏土",
						count: 0
					},
					{
						url: ("https://app.bowu66.com/museum-cms/upload/nansongGame/diglett/stone3.png"),
						name: "石英",
						count: 0
					},
					{
						url: ("https://app.bowu66.com/museum-cms/upload/nansongGame/diglett/stone4.png"),
						name: "白云母",
						count: 0
					},
					{
						url: ("https://app.bowu66.com/museum-cms/upload/nansongGame/diglett/stone5.png"),
						name: "长石",
						count: 0
					},
					{
						url: ("https://app.bowu66.com/museum-cms/upload/nansongGame/diglett/stone6.png"),
						name: "瓷石",
						count: 0
					},
				],
				// neededaterialsMList: [//正确的材料列表
				//     ("https://app.bowu66.com/museum-cms/upload/nansongGame/diglett/stone2.png"),
				//     ("https://app.bowu66.com/museum-cms/upload/nansongGame/diglett/stone3.png"),
				// ],
				// 游戏完毕弹出框
				passShow: false, //通关弹出框
				gotoShow: true,
				refuseShow: false, //失败弹出框
			}
		},
		mounted() { //挂载结束状态
			this.$store.commit("setAudioPlay", 'play');

			function imgLoad(img, callback) {
				let timer = setInterval(function() {
					if(img.complete) {
						callback(img);
						clearInterval(timer)
					}
				}, 50)
			}
			imgLoad($(".animation-bg2>img")[0], () => {
				setTimeout(() => {
					$(".animation-bg2").addClass("animation2");
				}, 500);
				imgLoad($(".animation-bg7>img")[0], () => {
					setTimeout(() => {
						$(".animation-bg7").addClass("animation7");
					}, 500);
				});
			});
			imgLoad($(".animation-bg3>img")[0], () => {
				setTimeout(() => {
					$(".animation-bg3").addClass("animation3");
				}, 500);
			});
			imgLoad($(".animation-bg5>img")[0], () => {
				setTimeout(() => {
					$(".animation-bg5").addClass("animation5");
				}, 500);
			});

            // this.pauseGame(1);

        },
		components: { //模板
			countDownT,
			stepRule,
			pass,
			refuse
		},
		methods: { //事件
			// 游戏开始
			gameStart() {
				// this.animationShow = false;
				let that = this;
				this.$refs.countDownChild.countDownT(() => {
					// 开始倒计时
					that.countDown();
					// 随机位置出现材料
					that.canPlay = true;
					that.randomPlace();
				});
			},

			hideanimationShow() {
				this.animationShow = false;

			},
			restart() {
				window.clearInterval(this.moveTime);
				this.totalTime = 59;
				this.totalMs = 99;
				this.clickTotalCount = 0;
				this.correctCount = 0;
				this.accuracy = 0;
				this.refuseShow = false;
				this.initPlace();
				$(".active").removeClass("active");
				this.gameStart();
			},
			defaultGame() {
				this.$router.push('/diglett');
			},
			getRandom(to, from) {
				return Math.floor(Math.random() * to + from);
			},
			// 游戏时间倒计时
			countDown() {
				let that = this;
				t1 = window.setInterval(() => {
					if(that.totalMs > 0) {
						that.totalMs -= 1;
					} else {
						if(that.totalTime > 0) {
							that.totalTime -= 1;
							that.totalMs = 99;
							if(that.totalTime < 5) {
								document.getElementById("time_sound").play();
							}
						} else {
							that.initPlace();
							document.getElementById("time_sound").pause();
							that.canPlay = false;
							that.refuseShow = true;
							window.clearInterval(t1);
						}
					}
				}, 10);
			},
			// 随机位置
			randomPlace() {
				this.initPlace();
				if(this.canPlay) {
					let that = this;
					that.radomFun();
					that.moveTime = setInterval(() => {
						that.radomFun();
					}, that.stayTime);
				}
			},
			radomFun() {
				let that = this;
				if(this.canPlay) {
					this.initPlace();
					let radomCount = that.getRandom(5, 1);
					let radomPlace = [];
					if(radomCount > 1) {
						for(let i = 0; i < radomCount; i++) {
							radomPlace.push(that.getRandom(6, 0));
						}
					} else {
						radomPlace.push(that.getRandom(6, 0));
					}
					for(let j = 0; j < radomPlace.length; j++) {
						let randomPicObj = that.materialsList[that.getRandom((that.materialsList.length), 0)];
						that.diglettPlaceArr[radomPlace[j]].active = true;
						that.diglettPlaceArr[radomPlace[j]].url = randomPicObj.url;
						that.diglettPlaceArr[radomPlace[j]].name = randomPicObj.name;
						that.diglettPlaceArr[radomPlace[j]].canClick = true;
						that.diglettPlaceArr[radomPlace[j]].truePic = true;
						// if (that.neededaterialsMList.indexOf(that.diglettPlaceArr[radomPlace[j]].url) != -1) {
						//     that.diglettPlaceArr[radomPlace[j]].truePic = true;
						// } else {
						//     that.diglettPlaceArr[radomPlace[j]].truePic = false;
						// }
					}
				}
			},
			// 初始化位置
			initPlace() {
				this.diglettPlaceArr = [{
						active: false,
						url: ""
					},
					{
						active: false,
						url: ""
					},
					{
						active: false,
						url: ""
					},
					{
						active: false,
						url: ""
					},
					{
						active: false,
						url: ""
					},
					{
						active: false,
						url: ""
					},
				];
			},
			pauseGame(type) {
				this.canPlay = false;
				this.initPlace();

				window.clearInterval(t1);
				switch(type) {
					case 1:
						$(".mining-content").empty();
						this.materialsList.map((n, i) => {
							$(".mining-content").append("<div class='mining-item " + ('mining-item' + (i + 1)) + "'>" +
								"<div class='mining-pic-box'><img src='" + n.url + "'/></div>" +
								"<div class='mining-name'>" + n.name + "</div>" +
								"<div class='mining-count'>X " + n.count + "</div>" +
								"</div>");
						});
						this.passShow = true;
						break;
					case 2:
						this.refuseShow = true;
				}
			},
			// 点击事件
			placeClick(e) {
				let target = e.target;
				if(this.canPlay) {
					if(this.diglettPlaceArr[$(target).parent().index()].canClick) {
						if(target.className.indexOf("active") != -1) {
							if(target.className.indexOf("truePic") != -1) {
                                let diglettClickSound = "https://app.bowu66.com/museum-cms/upload/nansongGame/secondSound/second-click.mp3";
                                let mp3Player = new Audio(diglettClickSound);
                                mp3Player.play();
								this.correctCount += 1;
								this.clickTotalCount += 1;
								// this.accuracy = parseInt((this.needClickCorrectCount) / 100 * this.correctCount * 100);
								this.accuracy = (this.correctCount / parseInt(this.needClickCorrectCount)) * 100;
								if(this.accuracy >= 100) {
									this.pauseGame(1);
								}
							} else {
								this.clickTotalCount += 1;
								if(this.clickTotalCount - this.correctCount == 3) {
									this.pauseGame(2);
								}
							}
							let word = $(e.target).attr("data-name");
							let peopleCount = $(e.target).attr("data-index");
							this.materialsList[parseInt(peopleCount)].count += 1;
							let newDiv = $("<div class='add-counts' style='position: absolute;z-index: 10;color: #fff;left:" + e.clientX + "px;top:" + e.clientY + "px'>" + word + "+1</div>");
							newDiv.appendTo($(".diglett-box"));
							newDiv.animate({
								top: e.clientY - 20,
								opacity: 0
							}, 1000, () => {
								newDiv.css({
									display: "none"
								});
							});
							this.diglettPlaceArr[$(target).parent().index()].canClick = false;
						}
					}
				}
			},
			gotoPage() {
				// let yaoqingrenid = sessionStorage.getItem("yaoqingrenid");
				let that = this;
				sessionStorage.setItem('totalScore', that.totalTime > 50 ? (35 + that.$randomNum(1,3)) : that.totalTime);
				this.$router.push('/molding');
			}
		}
	}
</script>

<style scoped lang="less">
	@import "./animation.less";
	@import "./diglett.less";
	@import "./progress.css";
	@import "../../../assets/css/common.less";
</style>