<template>
	<div class="game_box">
		<!--<div style="width:20px;height:5px;position:absolute;z-index:10000;right:0;top:0;background-color:red;"></div>-->
		<div class="game_tips_box">
			<div class="game-step">
				<img src="https://app.bowu66.com/museum-cms/upload/nansongGame/img/step4.png" alt="">
			</div>
		</div>
		<!--游戏倒计时-->
		...
		<div class="progress_bar">
			<span :style="{transition:'width 0.5s linear',width:holdTime*100/durationTime+'%'}"></span>
		</div>
		<!--火龙窑-->
		<div class="dragonKiln">
			<img src="https://app.bowu66.com/museum-cms/upload/nansongGame/kiln/dragonKiln.png" alt="">
		</div>
		<!--火柴-->
		<div @click="warming" class="match-box">

		</div>
		<!--柴火堆-->
		<div class="pile-wood-box">
			<img src="https://app.bowu66.com/museum-cms/upload/nansongGame/kiln/pile-wood.png" alt="">
		</div>
		<!--火把-->
		<div class="fire_box">
			<div class="fireplace">
		...
		<!--温度计-->
		<div class="thermometer_box">
			<div class="thermometer_container">
				<img class="glassBody" src="https://app.bowu66.com/museum-cms/upload/nansongGame/kiln/thermometer.png" alt="">
				<!--红色-->
				<img class="redVertical lessBg" src="https://app.bowu66.com/museum-cms/upload/nansongGame/kiln/redVertical.jpg" :style="{transition:'height 0.5s linear',bottom:redBottomBg+'px', height:verticalHeight+'px'}">
				<div class="arrows_right" v-show="arrowsTop!=0" :style="{transition:'bottom 0.5s linear',bottom:arrowsTop+'px'}"></div>
			</div>
		</div>
		<!--温度过高-->
		<div class="excess-temperature-box">
			<div class="excess-temperature">温度过高注意降温!</div>
		</div>
		<!--温度过低-->
		<div class="toolow-temperature-box">
			<div class="toolow-temperature">温度过低注意升温!</div>
		</div>
		<!--通过-->
		<pass :passShow="passShow" :gotoShow="gotoShow" @gotoPage="gotoPage" passWord="true" nextStepWord="查看称号"></pass>
		<!--失败-->
		<refuse :refuseShow="refuseShow" :showDefault="true" @restart="restart" @defaultGame="defaultGame"></refuse>
		<!--规则-->
		<gameRule :stepRule="stepRule" :stepTips="stepTips" :stepLogo="stepLogo" :stepTitle="stepTitle" ref="gameRule" @gameStart="gameStart"></gameRule>
		<!--游戏开始倒计时-->
		<countDownT ref="countDownChild"></countDownT>
		<!--音效-->
		<audio id="tooHighFire" preload="auto">
			<source src="https://app.bowu66.com/museum-cms/upload/nansongGame/kiln/fire-too-high.mp3" type="audio/mp3">
		</audio>
	</div>
</template>

<script>
	var t1, downBg, holdTimeInterval;
	...
	export default {
		...
		data() {
			return {
				...
				// 失败
				refuseShow: false,
				matchPic: ("https://app.bowu66.com/museum-cms/upload/nansongGame/kiln/pile-wood.png")
			}
		},
		computed: {
			// 温度计宽高
			thermometer: () => {
				...
				let thermometerHeight = windowHeight * 0.6;
				return {
					thermometerHeight: thermometerHeight,
					glassBodyHeight: thermometerHeight - 64,
					galssBottomTop: thermometerHeight - 47,
					glass1500: 10,
					glass1300: (thermometerHeight - 64) / 20 * 2 + 10,
					glass1200: (thermometerHeight - 64) / 20 * 5 + 10,
					yellowBg: (thermometerHeight - 64) * 0.7 + 1
				}
			}
		},
		components: { //模板
		...
		mounted() {
			let that = this;
			let canPlay = false;
			var canShowTooLow=false;
			setTimeout(() => {
				var playHighFire = window.setInterval(() => {
					let totalHeight = $(".glassBody").height();
					if(totalHeight > 0) {
						let maxHeight = totalHeight - (totalHeight * 0.195 + totalHeight * 0.0825);
						if((that.verticalHeight > maxHeight * 0.4425) && (that.verticalHeight < maxHeight * 0.730) && (that.holdTime < that.durationTime)) {
							that.$pause($('#tooHighFire')[0]);
							$(".excess-temperature").removeClass("excess-temperature1");
                            $(".toolow-temperature").removeClass("toolow-temperature1");
                            canShowTooLow=true;
                        } else {
							if(that.verticalHeight > maxHeight * 0.730) {
								// if (!canPlay) {
								that.$bgInit($('#tooHighFire')[0]);
								if(!$(".excess-temperature").hasClass("excess-temperature1")) {
									$(".excess-temperature").addClass("excess-temperature1");
								}
                                $(".toolow-temperature").removeClass("toolow-temperature1");
								canPlay = true;
								// }
								...
								that.$pause($('#tooHighFire')[0]);
								$(".excess-temperature").removeClass("excess-temperature1");
                                console.log("toolow-temperature");
                                if(canShowTooLow){
                                    if(!$(".toolow-temperature").hasClass("toolow-temperature1")) {
                                        $(".toolow-temperature").addClass("toolow-temperature1");
                                    }
								}
                            }
						}
		...
		methods: { //事件
			...
			warming(e) {
				...
				//top:0.0825
				if(this.canPlay) {
					$(".match-box").addClass("changeSize");
					if(this.verticalHeight < maxHeight) {
						this.verticalHeight += (maxHeight * this.addScale);
						this.arrowsTop += (maxHeight * this.addScale);
					}
					let that = this;
					if(!downBg) {
						this.downBgFun()
					}
					let $div = $('<div class="match-item">' +
						'<img src="' + that.matchPic + '" alt="">' +
						'</div>');
					$div.appendTo('.match-box');
					setTimeout(() => {
						$div.remove();
					}, 400);
					setTimeout(() => {
						$(".match-box").removeClass("changeSize")
					}, 500);
			...
			fillHoldTime() {
				...
				let maxHeight = totalHeight - (totalHeight * 0.195 + totalHeight * 0.0825);
				let that = this;
				holdTimeInterval = window.setInterval(() => {
					if((that.verticalHeight > maxHeight * 0.4425) && (that.verticalHeight < maxHeight * 0.730) && (that.holdTime < that.durationTime)) {
						that.holdTime += 1;
					} else {
						if(that.holdTime == that.durationTime) {
							that.passShow = true;
							window.clearInterval(downBg);
							downBg = null;
							window.clearInterval(holdTimeInterval);
							window.clearInterval(t1);
						}
					}
				}, 1000);
			...
			// 下降
			downBgFun() {
				let totalHeight = this.thermometer.glassBodyHeight;
				let that = this;
				downBg = window.setInterval(() => {
					if(that.verticalHeight > 0) {
						that.verticalHeight -= (totalHeight * that.subtractScale);
						that.arrowsTop -= (totalHeight * that.subtractScale);
						if(that.verticalHeight < 0) {
							that.verticalHeight = 0;
							that.arrowsTop = 18;
						}
					} else {
						window.clearInterval(downBg);
    ...
    sessionStorage.setItem('totalScore', totalScore + (this.totalTime > 39 ? (35 + this.$randomNum(1,5)) : this.totalTime));
